...
class SKAKeyframe:
	...
	def read(self, file: BinaryIO) -> 'SKAKeyframe':
		...
		self.x, self.y, self.z, self.w, self.tan_x, self.tan_y, self.tan_z, self.tan_w = data
		return self
...
class SKA:
	...
	def read(self, file_name: str) -> 'SKA':
		...
		self.times = [unpack('f', reader.read(calcsize('f')))[0] for _ in range(self.time_count)]
		self.keyframes = [SKAKeyframe().read(reader) for _ in range(self.time_count)]
