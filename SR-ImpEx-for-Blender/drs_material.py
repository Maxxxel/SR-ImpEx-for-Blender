...
class DRSMaterial:
    ...
    def get_or_create_base_node_tree(self) -> bpy.types.NodeTree:
        ...
        if base_node_tree is None:
            base_node_tree = bpy.data.node_groups.new(
                base_node_tree_name, type="ShaderNodeTree")
    ...
    def setup_node_tree_inputs_outputs(self, node_tree: bpy.types.NodeTree) -> None:
        if bpy.app.version[0] == 3:  # pylint: disable=E1136
            ...
            node_tree.inputs.new(
                name="IN-Roughness [green]", type=SOCKET_COLOR)
            ...
            node_tree.interface.new_socket(
                name="IN-Color Map", in_out="INPUT", socket_type=SOCKET_COLOR, parent=None)
            node_tree.interface.new_socket(
                name="IN-Color Map Alpha", in_out="INPUT", socket_type=SOCKET_FLOAT, parent=None)
            node_tree.interface.new_socket(
                name="IN-Metallic [red]", in_out="INPUT", socket_type=SOCKET_COLOR, parent=None)
            node_tree.interface.new_socket(
                name="IN-Roughness [green]", in_out="INPUT", socket_type=SOCKET_COLOR, parent=None)
            node_tree.interface.new_socket(
                name="IN-Emission [alpha]", in_out="INPUT", socket_type=SOCKET_COLOR, parent=None)
            node_tree.interface.new_socket(
                name="IN-Normal Map", in_out="INPUT", socket_type=SOCKET_NORMAL, parent=None)
            node_tree.interface.new_socket(
                name="OUT-DRS Shader", in_out="OUTPUT", socket_type=SOCKET_SHADER, parent=None)
    ...
    def link_nodes(self) -> None:
        self.material.node_tree.links.new(self.group.outputs.get(
            "OUT-DRS Shader"), self.material_output.inputs.get("Surface"))
        self.node_tree.links.new(self.shader.outputs.get(
            "BSDF"), self.outputs.inputs.get("OUT-DRS Shader"))

        # Link default inputs to the shader
        self.node_tree.links.new(self.inputs.outputs.get(
            "IN-Color Map"), self.shader.inputs.get("Base Color"))
        self.node_tree.links.new(self.inputs.outputs.get(
            "IN-Color Map Alpha"), self.shader.inputs.get("Alpha"))
        self.node_tree.links.new(self.inputs.outputs.get(
            "IN-Metallic [red]"), self.shader.inputs.get("Metallic"))
        self.node_tree.links.new(self.inputs.outputs.get(
            "IN-Roughness [green]"), self.shader.inputs.get("Roughness"))
        self.node_tree.links.new(self.inputs.outputs.get(
            "IN-Emission [alpha]"), self.shader.inputs.get("Emission Strength"))
        if bpy.app.version[0] in [3]:  # pylint: disable=E1136
            self.node_tree.links.new(self.inputs.outputs.get(
                "IN-Emission [alpha]"), self.shader.inputs.get("Emission"))
        if bpy.app.version[0] in [4]:  # pylint: disable=E1136
            self.node_tree.links.new(self.inputs.outputs.get(
                "IN-Color Map"), self.shader.inputs.get("Emission Color"))

    def create_image_nodes(self) -> None:
        # Create empty image nodes for later use
        self.color_map_node = self.material.node_tree.nodes.new(
            'ShaderNodeTexImage')
        self.color_map_node.location = Vector((-700.0, 0.0))
        self.material.node_tree.links.new(self.color_map_node.outputs.get(
            "Color"), self.group.inputs.get("IN-Color Map"))
        self.material.node_tree.links.new(self.color_map_node.outputs.get(
            "Alpha"), self.group.inputs.get("IN-Color Map Alpha"))

        self.parameter_map_node = self.material.node_tree.nodes.new(
            'ShaderNodeTexImage')
        self.parameter_map_node.location = Vector((-700.0, -100.0))
        self.material.node_tree.links.new(self.parameter_map_node.outputs.get(
            "Alpha"), self.group.inputs.get("IN-Emission [alpha]"))

        separate_rgb_node = self.material.node_tree.nodes.new(
            'ShaderNodeSeparateColor')
        separate_rgb_node.location = Vector((-400.0, -100.0))
        self.material.node_tree.links.new(self.parameter_map_node.outputs.get(
            "Color"), separate_rgb_node.inputs.get("Color"))
        self.material.node_tree.links.new(
            separate_rgb_node.outputs[1], self.group.inputs.get("IN-Roughness [green]"))
        self.material.node_tree.links.new(
            separate_rgb_node.outputs[0], self.group.inputs.get("IN-Metallic [red]"))

        self.normal_map_node = self.material.node_tree.nodes.new(
            'ShaderNodeTexImage')
        self.normal_map_node.location = Vector((-700.0, -300.0))
        self.material.node_tree.links.new(self.normal_map_node.outputs.get(
            "Color"), self.group.inputs.get("IN-Normal Map"))

        # Normal map nodes setup
        self.normal_map_separate = self.node_tree.nodes.new(
        ...
        self.normal_map_combine = self.node_tree.nodes.new(
            "ShaderNodeCombineXYZ")
        self.normal_map_combine.location = Vector((-300.0, -100.0))

        # Link normal map nodes internally
        self.node_tree.links.new(self.inputs.outputs.get(
            "IN-Normal Map"), self.normal_map_separate.inputs.get("Vector"))
        self.node_tree.links.new(self.normal_map_separate.outputs.get(
            "Y"), self.normal_map_invert.inputs.get("Color"))
        self.node_tree.links.new(self.normal_map_invert.outputs.get(
            "Color"), self.normal_map_combine.inputs.get("Y"))
        self.node_tree.links.new(self.normal_map_separate.outputs.get(
            "X"), self.normal_map_combine.inputs.get("X"))
        self.node_tree.links.new(self.normal_map_separate.outputs.get(
            "Z"), self.normal_map_combine.inputs.get("Z"))
        self.node_tree.links.new(self.normal_map_combine.outputs.get(
            "Vector"), self.shader.inputs.get("Normal"))
