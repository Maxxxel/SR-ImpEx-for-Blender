...
try:
    from sr_impex.updater.addon_updater import Updater as updater
...
def get_user_preferences(context=None):
    ...
    if hasattr(context, "preferences"):
        ...
        prefs = context.preferences.addons.get(base_package, None)
        # Check preferences.addons
        for addon in context.preferences.addons:
            if addon.module == base_package:
                prefs = addon
                break
    if prefs:
...
def register(bl_info):
    ...
    updater.user = "Maxxxel"

    # Choose your own repository, must match git name for GitHUb and Bitbucket,
    # for GitLab use project ID (numbers only).
    updater.repo = "SR-ImpEx-for-Blender"

    # updater.addon = # define at top of module, MUST be done first

    # Website for manual addon download, optional but recommended to set.
    updater.website = "https://github.com/Maxxxel/SR-ImpEx-for-Blender.git"
