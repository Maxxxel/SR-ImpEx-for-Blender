...
def CreateAnimation(SkaFile: SKA, Armature: bpy.types.Armature, ArmatureObject, BoneList: list[DRSBone], SkeletonData: CSkSkeleton, AnimationFileName: str):
	...
	ArmatureObject.select_set(state=True)
	bpy.context.view_layer.objects.active = ArmatureObject

	FPS: int = 24
	TotalFrames = SkaFile.Length6

	# Create the Action
	Armature.animation_data_create()
	NewAction = bpy.data.actions.new(name=AnimationFileName)
	NewAction.use_fake_user = True
	Armature.animation_data.action = NewAction

	Scene.frame_start = 0
	Scene.frame_end = TotalFrames
	Scene.render.fps = FPS

	# Loop the Header of the SkaFile and Check if the Header.BoneId is found in the BoneList -> Bone.SkaIdentifier
	for Header in SkaFile.Headers:
		# Get the Bone
		Bone = next((Bone for Bone in BoneList if Bone.SKAIdentifier == Header.BoneId), None)

		if Bone is None or Bone.Name not in ArmatureObject.pose.bones:
			continue
		...
		if Header.FrameType == 0:
			# Create the F-Curves
			LocationX = NewAction.fcurves.new(data_path='pose.bones["' + Bone.Name + '"].location', index=0)
			LocationY = NewAction.fcurves.new(data_path='pose.bones["' + Bone.Name + '"].location', index=1)
			LocationZ = NewAction.fcurves.new(data_path='pose.bones["' + Bone.Name + '"].location', index=2)
		elif Header.FrameType == 1:
			RotationX = NewAction.fcurves.new(data_path='pose.bones["' + Bone.Name + '"].rotation_quaternion', index=0)
			RotationY = NewAction.fcurves.new(data_path='pose.bones["' + Bone.Name + '"].rotation_quaternion', index=1)
			RotationZ = NewAction.fcurves.new(data_path='pose.bones["' + Bone.Name + '"].rotation_quaternion', index=2)
			RotationW = NewAction.fcurves.new(data_path='pose.bones["' + Bone.Name + '"].rotation_quaternion', index=3)

		# Loop through the Frames
		for FrameIndex in range(Header.Tick, Header.Tick + Header.Interval):
			FrameData = SkaFile.KeyframeData[FrameIndex]
			VectorData = FrameData.VectorData
			...
			if Header.FrameType == 0:
				# Translation
				Translation: Vector = Vector((VectorData.x, VectorData.y, VectorData.z))
				Translation = PoseBone.bone.matrix_local.inverted() @ Translation

				# Add the Keyframe
				LocationX.keyframe_points.insert(FrameIndex, Translation.x)
				LocationY.keyframe_points.insert(FrameIndex, Translation.y)
				LocationZ.keyframe_points.insert(FrameIndex, Translation.z)
			elif Header.FrameType == 1:
				# Rotation
				Rotation: Quaternion = Quaternion((VectorData.w, VectorData.x, VectorData.y, VectorData.z))
				Rotation = PoseBone.bone.matrix_local.inverted().to_quaternion() @ Rotation

				# Add the Keyframe
				RotationX.keyframe_points.insert(FrameIndex, Rotation.x)
				RotationY.keyframe_points.insert(FrameIndex, Rotation.y)
				RotationZ.keyframe_points.insert(FrameIndex, Rotation.z)
				RotationW.keyframe_points.insert(FrameIndex, Rotation.w)
