...
# AnimationType = {
#     "CastResolve": 0,
#     "Spawn": 1,  # Always needed
#     "Melee": 2,  # If unit has melee attacks
#     "Channel": 3,  # If unit has channeling abilities
#     "ModeSwitch": 4,  # only needed for units with different 'modes'
#     "WormMovement": 5,  # Only needed for worms
# }
...
#     # for #9-13 a building can only have a maximum of 8 of these total
# }
...
#         elif revision == 131073:
#             data = unpack_data(file, "fff", "ff")
...
#             self.normal = [0.0, 0.0, 0.0]
#         else:
#             raise ValueError(f"Unknown revision {revision} for Vertex")
...
#     z: float = 0.0
#     w: float = 0.0
#     xyz: tuple = field(default_factory=lambda: ((0, 0, 0)))
...
#     math_matrix: tuple = ((0, 0, 0), (0, 0, 0), (0, 0, 0))
...
#     super_parent: Matrix4x4 = field(default_factory=Matrix4x4)
...
#             raise TypeError(
#                 f"Unknown Material {self.unknown} identifier {self.identifier}"
#             )
#         return self
...
#         self.material_id = unpack("=h", file.read(calcsize("=h")))[0]
#         self.material_parameters = unpack("i", file.read(calcsize("i")))[0]
...
#         else:
#             raise TypeError(
#                 f"Unknown MaterialParameters {self.material_parameters}, material_id {self.material_id}"
#             )
#         return self
...
#             self.mode = unpack("h", file.read(calcsize("h")))[0]
...
#             node.name = node.name.replace("_node", "")
#             self.nodes.append(node)
